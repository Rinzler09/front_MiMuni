...
import '../../../style/UserInfoStyles/CambioContraseña.css';//Estilo para el componente de RestablecerContraseñaSesion.tsx
...
import {cambioContraseniaServices} from '../../../services/EnvioCorreoElectronicoServices'; //Servicio para enviar la solicitud de cambio de contraseña
import type {cambioContrasena} from '../../../types/generalForm'//Importancion del type para el manejo de los datos del formulario

const CambioContrasenaUsuario: React.FC = () => {
...
//en este caso lo que viene de type en generalForm.ts con los objetos de cambioContraseña.
  const initialValues: cambioContrasena = {contrasenaAnterior: "", contrasena: "", confirmaContra: "", };
  
  
  //el register permite en poder registrar los campos para poder validarlos
  //el formState es para manejar estados personalizados como en este caso los errores de los campos
  ...
  /*Esta funcion de handleCambioContraseña se hace responsable del evento para ejecutar el cambio de contraseña
  ...
  */
  const handleCambioContraseña = async (formData: cambioContrasena) =>{
    try {
        ...
        const response = await cambioContraseniaServices(formData.contrasenaAnterior, formData.contrasena, formData.confirmaContra);
    } catch (error) {
  ...
  return (
    ...
    */
    <div className="password-change-container mx-auto my-2">
      ...
      */}
       <form onSubmit={handleSubmit(handleCambioContraseña)}>
        ...
        */}
  <div className="mb-3 position-relative"> 
          <input type={showCurrent ? 'password' : 'text'} className=" form-control"  placeholder="Ingrese la contraseña Anterior"
             {...register("contrasenaAnterior", {
                    ...
                    hasAtLeastOneDigit:(value: string) =>{//La funcion de hasAtLeastOneDigit es donde recibe el valor del campo para validar
                        const digitCount = (value.match(/\d/g) || []).length;//este arreglo en donde devuelve string en donde si no encuentra un caracter
                         //evuelve null como vacio, ya que el operador logico 
                        return(digitCount >=1 || "La contraseña debe contener al menos un numero.");// Retorna en la parte de digitCount que nos dice 
                    ...
                    hasOneSpecial:(value: string) =>{
                        const specialCount = (value.match(/[-!@#$%^&*()_=+]/g) || []).length;//tene declarada el specialCount donde tiene el arreglo
                        ...
                        return(specialCount >= 1 || "Por seguridad, la contraseña debe incluir al menos uno de estos caracteres -!@#$%^&*()_=+");
        ...
        <div className="mb-3 position-relative">
           <div className="mb-3 position-relative">
          <input type={showNew ? 'password' : 'text'} className="form-control " placeholder="Ingrese la contraseña Nueva"
            {...register("contrasena", {
                    ...
                    hasOneSpecial:(value: string) =>{// La funcion nos valida si no hay campos vacios ya que se personaliza en distintas parte
                        const specialCount = (value.match(/[-!@#$%^&*()_=+]/g) || []).length;//tene declarada el specialCount donde tiene el arreglo
                        //en done valida los caractares especiales personalizado y tambien a su vez valida si no viene ningun vacio.
                        return(specialCount >= 1 || "Por seguridad, la contraseña debe incluir al menos uno de estos caracteres -!@#$%^&*()_=+");// En esta parte del codigo
                        //es fundamente porque valida la longitud del array ya que compara si specialCount >= 1 queriendo decir que podemos agregar mas caracteres,
                        //En este caso tuve que cambiar de posicion el carecter guion porque me afectaba en la validacion de la contraseña
                        //pero si no encuentra ninguno se activara el mensaje 
                    },
        ...
        <div className="mb-3 position-relative">
             <div className="mb-3 position-relative">
          <input type={showConfirm ? 'password' : 'text'} className="form-control " placeholder="Repita la contraseña"
